...
def detect_keypoints(imagename, threshold):
    ...
    # In each of the following for loops,
    # elements of each pyramids that are larger or smaller than its 26 immediate neighbors in space and scale
    # are labeled as extrema.
    # As explained in section 4 of Lowe's paper,
    # these initial extrema are pruned by checking that their contrast and curvature are above certain thresholds.
